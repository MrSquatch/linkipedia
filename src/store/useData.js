...
const useData = create((set, get) => ({
  ...
  nodeMap: new Map(),
  fetchState: null,
  selectedNode: null, // Nuevo estado para almacenar el nodo seleccionado
    ...
    try {
      ...
      bindings.forEach(({ property, item }) => {
        ...
        if (!nodeMap.has(item.id)) {
          ...
          if (
            !expandedItems.some((expandedItem) => expandedItem.id === item.id)
          ) {
  ...
  // Función para establecer el nodo seleccionado
